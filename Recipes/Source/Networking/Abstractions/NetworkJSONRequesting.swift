...
protocol NetworkJSONRequesting where Self: ClientNetworking & JSONCoding {
...
//  MARK: - Default Implementation (Private API)

private extension NetworkJSONRequesting {

    private func prepareAndSendRequest(_ endpoint: any Endpoint) async throws -> Data {
        let request = try endpoint.urlRequest(using: encoder)
